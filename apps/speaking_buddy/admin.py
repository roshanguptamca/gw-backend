...
class BuddyProfileAdmin(admin.ModelAdmin):
    list_display = ("id", "user", "buddy_name", "native_language", "target_language", "speaking_level", "is_memory_enabled", "created_at", "updated_at")
    search_fields = ("user__username", "user__email", "buddy_name", "learning_goal")
    list_filter = ("native_language", "target_language", "speaking_level", "is_memory_enabled", "preferred_correction_style")
...
class BuddySettingsAdmin(admin.ModelAdmin):
    list_display = ("id", "profile", "personality", "voice_style", "voice_gender", "voice_age", "correction_level", "difficulty_level", "created_at", "updated_at")
    search_fields = ("profile__user__username", "profile__user__email", "default_topic")
    list_filter = ("personality", "voice_style", "voice_gender", "voice_age", "correction_level", "difficulty_level")
    readonly_fields = ("created_at", "updated_at")


@admin.register(BuddyAvatar)
class BuddyAvatarAdmin(admin.ModelAdmin):
    list_display = ("id", "profile", "avatar_type", "name", "consent_confirmed", "is_active", "created_at", "updated_at")
...
class BuddySessionAdmin(admin.ModelAdmin):
    ...
    list_filter = ("language", "status", "started_at", "ended_at")
    readonly_fields = ("created_at", "updated_at")
...
class BuddyVocabularyAdmin(admin.ModelAdmin):
    list_display = ("id", "profile", "word", "language", "confidence_score", "last_practiced_at", "created_at", "updated_at")
...
class BuddyMistakeAdmin(admin.ModelAdmin):
    list_display = ("id", "profile", "session", "mistake_type", "language", "created_at")
    search_fields = ("profile__user__username", "profile__user__email", "mistake_type", "original_text", "corrected_text")
