...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--topic', type=str, default=None,
                            help='Process only this topic slug. Omit for all topics.')
        parser.add_argument('--batch-size', type=int, default=30,
                            help='Questions per translation batch (default 30)')
        parser.add_argument('--sleep', type=float, default=0.3,
                            help='Seconds between batches (default 0.3)')
        parser.add_argument('--dry-run', action='store_true',
                            help='Preview without saving')
        parser.add_argument('--reset-topic', type=str, default=None,
                            help='Clear NL content for a topic then re-translate')
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(style(
            f'\n{mode}Finished — {q_done} translated'
            + (f', {q_failed} failed' if q_failed else '')
        ))
